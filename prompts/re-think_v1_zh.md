...
### Step B-1.3 — 种子权重评估 (Seed Weight Evaluation — $K_{weight}$)

在进入扩展阶段前，必须先评估种子 $K$ 的信息权重：
...
**软停止扩一步操作步骤：**
1. 输出 2–3 条极简路径草图（每条仅一句话）——足够让用户建立基本方向感。
2. 提出 **1 个目标性澄清问题**：即那个能最大幅度降低路径间歧义度的定制变量。
3. **停止等待 (HALT)。** 不得进入完整扩展阶段，必须等待用户回应。
...
5.  **软停止针对的是稀缺的 $K$，而非模糊的 $G$。**
    在 PROT_B 下，目标 $G$ 的模糊或缺失绝不是停止运行的理由——那正是拓宽搜索范围（而非收窄）的信号。但当种子 $K$ 稀缺到无法在各路径间得出有意义区分时，则应触发软停止 (SOFT STOP)：输出路径草图、提出一个问题、等待用户回应。原则：当模型缺少用于判断*哪条路径更重要*的变量时停止——而非当目标终点不第一时间明确时停止。
...
> **说明：** 若 $K_{weight}$ = 稀缺，则在 $K_{weight}$ 行处截断标头块。输出 2–3 条路径草图（每条仅一句话）并提出 1 个澄清问题。在用户回应前，不得输出 $\Delta$ 行或 EXP 行。
...
4.  **HARD STOP / SOFT STOP 均会截断标头块：** 若 $\Delta$ 导致 HARD STOP（PROT_A）——在 Delta 行处截断，不输出验证行，此时尚无解决方案。若 $K_{weight}$ = 稀缺（PROT_B）——在 $K_{weight}$ 行处截断，同样不输出验证行/扩展行。
